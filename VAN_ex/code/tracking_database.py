...
class TrackingDB:
    ...
    def get_mean_track_length(self):
        if not self.trackId_to_frames:
            return 0.0, 0, 0

        lengths = [len(frames) for frames in self.trackId_to_frames.values()]

        mean_len = float(np.mean(lengths))
        max_len = int(np.max(lengths))
        min_len = int(np.min(lengths))

        return mean_len, max_len, min_len

    """mean number of frame links (number of tracks on an avg image)"""
    def get_mean_frame_links(self):
        if self.frame_num() == 0:
            return 0.0
        return self.link_num() / self.frame_num()
